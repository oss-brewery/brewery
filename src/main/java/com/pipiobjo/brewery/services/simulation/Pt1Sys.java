...
public class Pt1Sys {
    ...
    public BigDecimal calculate(BigDecimal stepSizeBD, BigDecimal input){
        ...
        xdot = kGain.multiply(input).subtract(x).divide(timeConstant, 15, RoundingMode.HALF_DOWN);
        x = x.add(stepSizeBD.multiply(xdot).divide(BigDecimal.valueOf(1000), 15, RoundingMode.HALF_DOWN));
        return x;
    }
}
